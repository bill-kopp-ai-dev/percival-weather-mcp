...
# ``docs/mcp/percival-weather-mcp/oci-label.json`` (regenerated by
# ``uv run python -m percival_weather_mcp.tool_export --registry`` and the
# ``build_oci_label.py`` helper). It is duplicated here as a literal so the
# image stays self-contained at build time and the gateway can introspect it
# without launching the container. ``tests/test_dockerfile.py`` and
# ``tests/test_oci_label.py`` assert that the embedded label matches the
# JSON source.
#
