...
with a YAML payload matching `catalog.ImportedServer` exactly. The
payload is templated from `mcp.yaml` at build time so the two stay in
sync (a CI step fails the build if they diverge).
...
| `tests/test_dockerfile.py`      | lint Dockerfile + entrypoint (image, label, ENTRYPOINT, CMD, etc.)     |
